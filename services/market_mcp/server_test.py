...
class TestMarketMcpServer:
    ...
    async def test_get_volatility(self, server, influxdb_client):
        ...
        response = json.loads(result.content[0].text)
        assert response["volatility"] == 0.0023
        assert response["atr"] == 150.5
    ...
    async def test_get_symbols(self, server, redis_client):
        ...
        redis_client.get_symbols.return_value = [
            {"id": "btcusd", "asset_class": "crypto", "base": "BTC", "quote": "USD", "exchange": "multi"},
            {"id": "ethusd", "asset_class": "crypto", "base": "ETH", "quote": "USD", "exchange": "multi"},
        ]
