...
flags.DEFINE_integer(
    "min_strategies_for_frequency",
    5,
    "Minimum strategies needed for frequency calculation",
)
flags.DEFINE_integer(
    "recency_days", 30, "Number of days to consider for recency calculation"
)

# Output Configuration
flags.DEFINE_string(
    "output_topic", "strategy-confidence-scores", "Kafka topic for confidence scores"
)
...
class StrategyConfidenceScorer:
    ...
    async def get_strategies_with_confidence(
        self, symbol: Optional[str] = None
    ) -> List[StrategyConfidenceScore]:
    ...
    def _calculate_confidence_score(
        ...
        total_confidence = (
            performance_component + frequency_component + recency_component
        )
    ...
    def print_confidence_scores(
        self, scores: List[StrategyConfidenceScore], limit: int = 20
    ):
        ...
        print(f"{'='*80}")
        print(
            f"{'Symbol':<12} {'Strategy Type':<20} {'Score':<8} {'Confidence':<12} {'Freq':<6} {'Days':<6}"
        )
        print(f"{'-'*80}")

        for score in scores[:limit]:
            print(
                f"{score.symbol:<12} {score.strategy_type:<20} "
    ...
    async def run():
        try:
            ...
            if confidence_scores:
                avg_confidence = sum(
                    s.confidence_score for s in confidence_scores
                ...
                for symbol in sorted(symbols)[:5]:  # Show first 5 symbols
                    ...
                    if symbol_scores:
                        ...
                        print(
                            f"{symbol}: {top_strategy.strategy_type} (confidence: {top_strategy.confidence_score:.3f})"
                        )
