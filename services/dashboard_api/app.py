...
def create_app(db_pool: asyncpg.Pool) -> FastAPI:
    ...
    async def get_overview():
        ...
        try:
            async with db_pool.acquire() as conn:
                # Portfolio value and unrealized PnL
                ...
                # Realized PnL by period
                pnl_row = await conn.fetchrow("""
                    SELECT
                        COALESCE(SUM(pnl), 0) AS total_realized_pnl,
                        COALESCE(SUM(CASE WHEN closed_at >= NOW() - INTERVAL '1 day' THEN pnl ELSE 0 END), 0) AS pnl_today,
                        COALESCE(SUM(CASE WHEN closed_at >= NOW() - INTERVAL '7 days' THEN pnl ELSE 0 END), 0) AS pnl_week,
                        COALESCE(SUM(CASE WHEN closed_at >= NOW() - INTERVAL '30 days' THEN pnl ELSE 0 END), 0) AS pnl_month
                    FROM paper_trades
                    WHERE status = 'CLOSED'
                """)

                # Active strategies count
                strategies_row = await conn.fetchrow("""
                ...
                positions_row = await conn.fetchrow("""
                    SELECT COUNT(*) AS open_positions
                    FROM paper_portfolio
                    WHERE quantity != 0
                """)

                # Pending alerts (unread signals)
            ...
            async with db_pool.acquire() as conn:
                rows = await conn.fetch(f"""
                    ...
                    SELECT
                        t.ts,
                        COALESCE(
                            (SELECT te.cumulative_pnl FROM trade_equity te WHERE te.ts <= t.ts ORDER BY te.ts DESC LIMIT 1),
                            0
                        ) AS value
                    FROM time_series t
                    ORDER BY t.ts
    ...
    async def get_top_strategies():
        ...
        try:
            async with db_pool.acquire() as conn:
                # Get top strategies by sharpe ratio from performance table
                ...
                for row in strategies:
                    impl_id = row["id"]
                    # Sparkline: daily returns over last 30 days
    ...
    async def get_market_summary():
        ...
        try:
            async with db_pool.acquire() as conn:
                # Get distinct symbols from portfolio and active strategies
                ...
                # Get latest signals as a proxy for price activity
                market_data = []
                for sym_row in symbols:
                    ...
                    if latest and latest["price"]:
                        ...
                        prev_price = float(prev["price"]) if prev and prev["price"] else current_price
