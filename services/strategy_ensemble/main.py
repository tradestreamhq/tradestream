...
flags.DEFINE_integer(
    "max_strategies_per_ensemble", 5, "Maximum strategies per ensemble"
)
flags.DEFINE_integer(
    "min_strategies_per_ensemble", 2, "Minimum strategies per ensemble"
)
flags.DEFINE_float("min_ensemble_score", 0.7, "Minimum ensemble score to consider")
flags.DEFINE_float(
    "diversification_weight", 0.3, "Weight for strategy type diversification"
)
flags.DEFINE_float(
    "performance_weight", 0.7, "Weight for individual strategy performance"
)
flags.DEFINE_boolean(
    "require_diversification", True, "Require different strategy types in ensemble"
)

# Output Configuration
flags.DEFINE_string(
    "output_topic", "strategy-ensembles", "Kafka topic for strategy ensembles"
)
...
class StrategyEnsembleBuilder:
    ...
    def build_ensembles(self, strategies: List[Dict]) -> List[StrategyEnsemble]:
        ...
        for symbol, symbol_strategies in strategies_by_symbol.items():
            symbol_ensembles = self._build_ensembles_for_symbol(
                symbol, symbol_strategies
            )
            ensembles.extend(symbol_ensembles)

        return ensembles

    def _build_ensembles_for_symbol(
        self, symbol: str, strategies: List[Dict]
        ...
        for ensemble_size in range(
            FLAGS.min_strategies_per_ensemble,
    ...
    def _calculate_ensemble_metrics(self, strategies: List[Dict]) -> Dict:
        ...
        diversification_score = (
            len(strategy_types) / max_possible_types if max_possible_types > 0 else 0
        )
    ...
    def print_ensembles(self, ensembles: List[StrategyEnsemble], limit: int = 20):
        ...
        print(f"{'='*120}")
        print(
            f"{'Symbol':<12} {'Ensemble ID':<25} {'Total Score':<12} {'Perf Score':<12} {'Div Score':<12} {'Risk Score':<12} {'Strategies':<20}"
        )
        print(f"{'-'*120}")
        ...
        for ensemble in ensembles[:limit]:
            ...
            print(
                f"{ensemble.symbol:<12} {ensemble.ensemble_id:<25} "
    ...
    def print_ensemble_details(self, ensemble: StrategyEnsemble):
        ...
        for strategy in ensemble.strategies:
            print(
                f"{strategy.strategy_type:<50} {strategy.current_score:<10.3f} "
        ...
        try:
            ...
            if ensembles:
                ...
                avg_performance_score = sum(
                    e.performance_score for e in ensembles
                ) / len(ensembles)
                avg_diversification_score = sum(
                    e.diversification_score for e in ensembles
                ) / len(ensembles)
                ...
                for symbol in sorted(symbols)[:5]:
                    ...
                    if symbol_ensembles:
                        ...
                        print(
                            f"{symbol}: {len(top_ensemble.strategies)} strategies (score: {top_ensemble.total_score:.3f})"
                        )
