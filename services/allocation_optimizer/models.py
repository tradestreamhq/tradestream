...
class StrategyReturns(BaseModel):
    ...
    returns: List[float] = Field(..., description="Historical period returns (e.g. daily)")
...
class OptimizationRequest(BaseModel):
    ...
    min_weight: float = Field(0.0, ge=0.0, le=1.0, description="Min allocation per strategy")
    max_weight: float = Field(1.0, ge=0.0, le=1.0, description="Max allocation per strategy")


class OptimizationResult(BaseModel):
    """Result of a portfolio optimization."""
    weights: Dict[str, float] = Field(..., description="Optimal allocation weights per strategy")
    expected_return: float = Field(..., description="Expected portfolio return (per period)")
    volatility: float = Field(..., description="Expected portfolio volatility (per period)")
