...
flags.DEFINE_string(
    "output_topic", "risk-adjusted-positions", "Kafka topic for position sizes"
)
...
class RiskAdjustedSizer:
    ...
    def calculate_risk_parity_size(
        self, volatility: float, target_risk: float
    ) -> float:
    ...
    def calculate_volatility_adjusted_size(
        self, score: float, volatility: float
    ) -> float:
    ...
    def calculate_position_sizes(
        ...
        for metrics in risk_metrics:
            ...
            risk_parity_size = self.calculate_risk_parity_size(
                metrics.volatility, target_risk
            )
            ...
            expected_return = (
                metrics.current_score * final_size - FLAGS.risk_free_rate * final_size
            )
    ...
    def print_position_sizes(self, position_sizes: List[PositionSize], limit: int = 20):
        ...
        print(f"{'='*140}")
        print(
            f"{'Symbol':<12} {'Strategy Type':<20} {'Confidence':<12} {'Kelly':<8} {'Risk Parity':<12} {'Vol Adj':<10} {'Final':<8} {'Risk':<8} {'Return':<10}"
        )
        print(f"{'-'*140}")

        for position in position_sizes[:limit]:
            print(
                f"{position.symbol:<12} {position.strategy_type:<20} "
...
def main(argv):
    ...
    async def run():
        try:
            ...
            if position_sizes:
                avg_final_size = sum(p.final_size for p in position_sizes) / len(
                    position_sizes
                )
                avg_risk_contribution = sum(
                    p.risk_contribution for p in position_sizes
                ) / len(position_sizes)
                avg_expected_return = sum(
                    p.expected_return for p in position_sizes
                ) / len(position_sizes)
                ...
                for symbol in sorted(symbols)[:5]:
                    ...
                    if symbol_positions:
                        ...
                        print(
                            f"{symbol}: {top_position.strategy_type} "
                ...
                for symbol in sorted(symbols)[:5]:
                    symbol_positions = [p for p in position_sizes if p.symbol == symbol]
                    total_symbol_risk = sum(
                        p.risk_contribution for p in symbol_positions
                    )
