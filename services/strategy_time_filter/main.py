...
flags.DEFINE_integer(
    "max_strategy_age_days", 90, "Maximum age of strategies to consider"
)
flags.DEFINE_integer(
    "min_backtest_window_days", 30, "Minimum backtesting window required"
)
flags.DEFINE_integer(
    "max_backtest_window_days", 365, "Maximum backtesting window allowed"
)
flags.DEFINE_float(
    "time_decay_factor", 0.1, "Performance decay per month after discovery"
)
flags.DEFINE_boolean(
    "require_recent_discovery", True, "Require strategies to be discovered recently"
)

# Output Configuration
flags.DEFINE_string(
    "output_topic", "time-filtered-strategies", "Kafka topic for filtered strategies"
)
...
class StrategyTimeFilter:
    ...
    async def get_time_filtered_strategies(
        self, symbol: Optional[str] = None
    ) -> List[TimeFilteredStrategy]:
                    ...
                    filtered_strategies.append(
                        TimeFilteredStrategy(
                            ...
                            backtest_window_days=(
                                int(row["backtest_window_days"])
                                if row["backtest_window_days"]
                                else 0
                            ),
    ...
    def _apply_time_filters(
        ...
        if backtest_window_days is not None:
            is_valid_window = (
                backtest_window_days >= FLAGS.min_backtest_window_days
                and backtest_window_days <= FLAGS.max_backtest_window_days
            )
    ...
    def print_filtered_strategies(
        self, strategies: List[TimeFilteredStrategy], limit: int = 20
    ):
        ...
        print(f"{'='*100}")
        print(
            f"{'Symbol':<12} {'Strategy Type':<20} {'Score':<8} {'Adj Score':<10} {'Days':<6} {'Window':<8} {'Recent':<8} {'Valid':<6}"
        )
        print(f"{'-'*100}")

        for strategy in strategies[:limit]:
            print(
                f"{strategy.symbol:<12} {strategy.strategy_type:<20} "
    ...
    async def get_filtering_statistics(self) -> Dict:
        ...
        async with self.pool.acquire() as conn:
            ...
            """
            window_stats = await conn.fetchrow(
                window_query,
                FLAGS.min_backtest_window_days,
                FLAGS.max_backtest_window_days,
            )
...
def main(argv):
    ...
    async def run():
        try:
            ...
            print(
                f"Recent strategies (≤{FLAGS.max_strategy_age_days} days): {stats['recent_strategies']}"
            )
            print(
                f"Old strategies (> {FLAGS.max_strategy_age_days} days): {stats['old_strategies']}"
            )
            ...
            if filtered_strategies:
                avg_adjusted_score = sum(
                    s.time_adjusted_score for s in filtered_strategies
                ) / len(filtered_strategies)
                max_adjusted_score = max(
                    s.time_adjusted_score for s in filtered_strategies
                )
                min_adjusted_score = min(
                    s.time_adjusted_score for s in filtered_strategies
                )
                ...
                for symbol in sorted(symbols)[:5]:
                    symbol_strategies = [
                        s for s in filtered_strategies if s.symbol == symbol
                    ]
                    symbol_strategies.sort(
                        key=lambda x: x.time_adjusted_score, reverse=True
                    )
                    if symbol_strategies:
                        top_strategy = symbol_strategies[0]
                        print(
                            f"{symbol}: {top_strategy.strategy_type} (adjusted score: {top_strategy.time_adjusted_score:.3f})"
                        )
