"""Entry point for the Portfolio State Awareness service."""
...
flags.DEFINE_float("initial_capital", 10000.0, "Initial paper trading capital")
...
def main(argv):
    ...
    loop_thread = threading.Thread(
        target=loop.run_forever, daemon=True, name="portfolio-state-loop"
    )
    loop_thread.start()
    ...
    flask_app = create_app(
        ...
        initial_capital=FLAGS.initial_capital,
        event_loop=loop,
    )
    ...
    logging.info("Portfolio State service starting on port %d", FLAGS.port)
