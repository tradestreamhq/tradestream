...
class CandleProcessorTest(unittest.TestCase):
    ...
    def test_window_size_calculation(self):
        ...
        # This test previously checked the count of requests generated by the *last* candle addition.
        # With the accumulation, we check the distinct window sizes from the last set of requests.
        last_candle_requests = processor_5min.add_candle(create_test_candle(currency_pair, timestamp_ms=get_candle_timestamp_ms(candles[-1]) + 60000*5 )) # ensure new candle
        
        # Re-add the candles to correctly get the final set of requests from the last add_candle call
        processor_5min = CandleProcessor(
            fibonacci_windows_minutes=[10, 15],
            deque_maxlen=50,
            default_strategy_type=StrategyType.SMA_RSI,
        ...
        for i in range(len(candles) -1):
            processor_5min.add_candle(candles[i])
        final_add_requests = processor_5min.add_candle(candles[-1])

        window_sizes = sorted([len(req.candles) for req in final_add_requests])
