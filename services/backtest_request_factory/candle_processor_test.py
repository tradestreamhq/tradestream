...
class CandleProcessorTest(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(self.processor.default_strategy_type, self.default_strategy_type)
        self.assertEqual(self.processor.default_strategy_parameters_any, self.default_parameters)
        self.assertEqual(self.processor.candle_granularity_minutes, self.candle_granularity_minutes)
    ...
    def test_add_candle_insufficient_candles(self):
        ...
        for i in range(3):
            candle = create_test_candle(currency_pair, timestamp_ms=1640995200000 + i * 60000)
            requests = self.processor.add_candle(candle)
            self.assertEqual(len(requests), 0)
    ...
    def test_add_candle_multiple_windows(self):
        ...
        self.assertEqual(len(final_requests), 4)  # All 4 windows should generate requests
    ...
    def test_add_candle_out_of_order_warning(self):
        ...
        # Add first candle
        candle1 = create_test_candle(currency_pair, timestamp_ms=1640995260000)  # Later time
        self.processor.add_candle(candle1)
        
        # Add second candle with earlier timestamp
        candle2 = create_test_candle(currency_pair, timestamp_ms=1640995200000)  # Earlier time
        
        with patch('services.backtest_request_factory.candle_processor.logging') as mock_logging:
