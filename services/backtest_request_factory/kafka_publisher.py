...
kafka_retry_params = dict(
    ...
    retry=retry_if_exception_type((kafka.errors.KafkaConnectionError, kafka.errors.KafkaTimeoutError)),
    reraise=True,
)
...
class KafkaPublisher:
    ...
    def _connect_with_retry(self):
        try:
            ...
            logging.info(f"Successfully connected KafkaProducer to {self.bootstrap_servers}")
        except kafka.errors.NoBrokersAvailable as e:
            logging.error(f"No Kafka brokers available at {self.bootstrap_servers}: {e}")
    ...
    def _publish_message_retryable(self, request_bytes: bytes, key_bytes: bytes = None):
        if not self.producer:
            ...
            if not self.producer: # If still None after retry, then fail
                 raise kafka.errors.KafkaConnectionError("Kafka producer could not be initialized after retry.")
        ...
        try:
            ...
            logging.info(f"Successfully published BacktestRequest to Kafka topic '{self.topic_name}', partition {record_metadata.partition}, offset {record_metadata.offset}")
    ...
    def publish_request(self, request: BacktestRequest, key: str = None):
        if not self.producer:
            logging.warning("Kafka producer not available. Attempting to send message might fail or retry.")
        ...
        try:
            ...
            logging.error(f"Failed to publish BacktestRequest to Kafka after all retries: {e}")
