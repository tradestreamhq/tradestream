...
class ConfigTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.original_env = {}
        env_vars = [
            'INFLUXDB_URL', 'INFLUXDB_TOKEN', 'INFLUXDB_ORG', 'INFLUXDB_BUCKET_CANDLES',
            'KAFKA_BOOTSTRAP_SERVERS', 'KAFKA_BACKTEST_REQUEST_TOPIC',
            'POLLING_INTERVAL_SECONDS', 'TOP_N_CRYPTOS', 'CMC_API_KEY',
            'DEFAULT_STRATEGY_TYPE', 'CANDLE_GRANULARITY_MINUTES'
        ]
        for var in env_vars:
    ...
    def test_fibonacci_windows_values(self):
        ...
        self.assertEqual(
            config.FIBONACCI_WINDOWS_MINUTES,
            sorted(config.FIBONACCI_WINDOWS_MINUTES)
        )
