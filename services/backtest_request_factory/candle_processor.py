...
class CandleProcessor:
    def __init__(self, fibonacci_windows_minutes: list[int], deque_maxlen: int,
                 default_strategy_type: StrategyType, default_strategy_parameters_any: any_pb2.Any = None,
        ...
        self.default_strategy_parameters_any = default_strategy_parameters_any or any_pb2.Any()
        self.candle_granularity_minutes = candle_granularity_minutes
        self.pair_deques: dict[str, collections.deque[Candle]] = {}
        logging.info(f"CandleProcessor initialized with Fibonacci windows (minutes): {self.fibonacci_windows_minutes}, deque maxlen: {self.deque_maxlen}")
    ...
    def add_candle(self, candle: Candle) -> list[BacktestRequest]:
        ...
        if currency_pair not in self.pair_deques:
            logging.warning(f"Received candle for uninitialized currency pair: {currency_pair}. Initializing deque.")
        ...
        if self.pair_deques[currency_pair]:
            last_candle_in_deque_ts_ms = self.pair_deques[currency_pair][-1].timestamp.seconds * 1000 + \
                                         self.pair_deques[currency_pair][-1].timestamp.nanos // 1_000_000
            new_candle_ts_ms = candle.timestamp.seconds * 1000 + candle.timestamp.nanos // 1_000_000
            if new_candle_ts_ms < last_candle_in_deque_ts_ms:
                logging.warning(f"Received out-of-order candle for {currency_pair}. New: {new_candle_ts_ms}, Last in deque: {last_candle_in_deque_ts_ms}. Appending anyway.")


        self.pair_deques[currency_pair].append(candle)
        logging.debug(f"Added candle to {currency_pair} deque. New deque size: {len(self.pair_deques[currency_pair])}")

        generated_requests = []
        current_deque = list(self.pair_deques[currency_pair]) # Work with a copy for windowing
        ...
        for window_minutes in self.fibonacci_windows_minutes:
            ...
            if window_size_candles <= 0: # Skip invalid window sizes
                logging.warning(f"Skipping invalid Fibonacci window size (candles): {window_size_candles} for {window_minutes} minutes.")
                continue
