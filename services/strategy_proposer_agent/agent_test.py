...
def _make_top_strategies():
    return [
        {
            "impl_id": "impl_1",
            "strategy_type": "momentum_ema_crossover",
            "score": 2.1,
        },
        {
            "impl_id": "impl_2",
            "strategy_type": "mean_reversion_bollinger",
            "score": 1.8,
        },
    ]
...
class TestCallMcpTool:
    ...
    def test_successful_mcp_call(self, mock_post):
        ...
        assert "momentum_ema_crossover" in parsed
...
class TestRunProposer:
    """Tests for the main proposer agent loop."""

    @mock.patch("requests.post")
    @mock.patch("services.strategy_proposer_agent.agent.OpenAI")
    def test_proposer_completes_with_spec(self, mock_openai_cls, mock_requests_post):
        """Test that the proposer completes a full workflow."""
        ...
        # Second LLM call finishes with a proposed spec
        proposed_spec = {
            "name": "adx_obv_trend_volume",
            "indicators": {
                "ADX": {"period": 14},
                "OBV": {},
                "EMA": {"period": 20},
            ...
            "entry_conditions": {
                ...
                "volume_rising": "OBV > OBV_prev",
                "price_above_ema": "close > EMA(20)",
            },
            "exit_conditions": {
                "trend_weakening": "ADX < 20",
                "price_below_ema": "close < EMA(20)",
            },
            "parameters": {
            ...
            "description": "Trend-following strategy using ADX for trend strength confirmation with OBV volume validation and EMA for direction.",
        }

        msg_2 = mock.Mock()
        msg_2.tool_calls = None
        msg_2.content = json.dumps(proposed_spec)
        ...
        parsed = json.loads(result)
        assert parsed["name"] == "adx_obv_trend_volume"
    ...
    @mock.patch("services.strategy_proposer_agent.agent.OpenAI")
    def test_proposer_handles_max_iterations(self, mock_openai_cls):
        """Test that the proposer stops after max iterations."""
        mock_client = mock.Mock()
        mock_openai_cls.return_value = mock_client
...
class TestSystemPrompt:
    ...
    def test_indicator_catalog_skill_present(self):
        ...
        assert "SMA" in agent.SYSTEM_PROMPT
    ...
    def test_gap_analysis_skill_present(self):
        assert "/gap-analysis" in agent.SYSTEM_PROMPT

    def test_model_is_sonnet(self):
        """Verify the agent uses Sonnet, not Haiku."""
        # Check the run_proposer function uses the right model
        import inspect
        source = inspect.getsource(agent.run_proposer)
