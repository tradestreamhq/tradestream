...
from services.strategy_proposer_agent.agent import run_agent
...
def main(argv):
    ...
    while not _shutdown:
        try:
            ...
            if result:
                logging.info("Strategy proposal result: %s", result[:500])
            else:
                logging.warning("No result from strategy proposer")
        except Exception as e:
            logging.exception("Error proposing strategy: %s", e)
