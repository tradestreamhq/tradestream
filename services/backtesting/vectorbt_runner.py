...
class VectorBTRunner:
    ...
    def run_strategy(
        ...
        entry_signal, exit_signal = self._generate_signals(ohlcv, strategy_name, parameters)
    ...
    def _sma_rsi_signals(self, ohlcv: pd.DataFrame, params: Dict) -> Tuple[pd.Series, pd.Series]:
    ...
    def _macd_crossover_signals(self, ohlcv: pd.DataFrame, params: Dict) -> Tuple[pd.Series, pd.Series]:
        ...
        macd = vbt.MACD.run(close, fast_window=fast, slow_window=slow, signal_window=signal_period)
    ...
    def _ema_macd_signals(self, ohlcv: pd.DataFrame, params: Dict) -> Tuple[pd.Series, pd.Series]:
        ...
        macd = vbt.MACD.run(close, fast_window=short_ema, slow_window=long_ema, signal_window=signal_period)
        ...
        exits = ema_bearish.diff().fillna(False) | (macd_bearish.diff().fillna(False) & macd_bearish)
        
        return entries.astype(bool), exits.astype(bool)
    
    def _sma_ema_crossover_signals(self, ohlcv: pd.DataFrame, params: Dict) -> Tuple[pd.Series, pd.Series]:
    ...
    def _rsi_ema_crossover_signals(self, ohlcv: pd.DataFrame, params: Dict) -> Tuple[pd.Series, pd.Series]:
    ...
    def _bbands_signals(self, ohlcv: pd.DataFrame, params: Dict) -> Tuple[pd.Series, pd.Series]:
        """Bollinger Bands mean reversion signals."""
        close = ohlcv['close']
        period = params.get('period', 20)
        mult = params.get('multiplier', 2.0)
        
        upper = self.registry.create("BOLLINGER_UPPER", close, ohlcv, {"period": period, "multiplier": mult})
        lower = self.registry.create("BOLLINGER_LOWER", close, ohlcv, {"period": period, "multiplier": mult})
        middle = self.registry.create("BOLLINGER_MIDDLE", close, ohlcv, {"period": period})
    ...
    def _adx_stochastic_signals(self, ohlcv: pd.DataFrame, params: Dict) -> Tuple[pd.Series, pd.Series]:
    ...
    def _double_ema_crossover_signals(self, ohlcv: pd.DataFrame, params: Dict) -> Tuple[pd.Series, pd.Series]:
    ...
    def _triple_ema_crossover_signals(self, ohlcv: pd.DataFrame, params: Dict) -> Tuple[pd.Series, pd.Series]:
        ...
        short_ema = self.registry.create("EMA", close, ohlcv, {"period": short_period})
        medium_ema = self.registry.create("EMA", close, ohlcv, {"period": medium_period})
        long_ema = self.registry.create("EMA", close, ohlcv, {"period": long_period})
    ...
    def _calc_sortino_ratio(self, returns: pd.Series) -> float:
        try:
            ...
            if len(negative_returns) == 0 or negative_returns.std() == 0:
                return 0.0
            excess_returns = returns.mean() - self._risk_free_rate / self._bars_per_year
            return float(excess_returns / negative_returns.std() * np.sqrt(self._bars_per_year))
        except:
            return 0.0
    ...
    def _with_strategy_score(self, metrics: BacktestMetrics) -> BacktestMetrics:
        ...
        def normalize(value: float, min_val: float = -1.0, max_val: float = 2.0) -> float:
