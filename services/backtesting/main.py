...
class BacktestHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _handle_batch(self):
        ...
        try:
            ...
            results = self.service.run_batch_backtest(candles, strategy_name, parameter_sets)
...
def run_http_server(port: int, service: BacktestingServiceImpl):
    ...
    logger.info(f"  POST /backtest - Run single backtest")
    logger.info(f"  POST /batch - Run batch backtests")
    logger.info(f"  GET /health - Health check")
    logger.info(f"  GET /indicators - List available indicators")
...
def main():
    ...
    parser.add_argument('--mode', choices=['http', 'grpc', 'both'], default='http',
                       help='Server mode')
    ...
    if args.mode == 'http':
        ...
        import threading
        http_thread = threading.Thread(target=run_http_server, args=(args.port, service))
