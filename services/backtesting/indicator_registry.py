...
class IndicatorRegistry:
    ...
    def _register_defaults(self) -> None:
        ...
        # Moving Averages
        self.register("SMA", lambda c, df, p: vbt.MA.run(c, window=p.get_int("period", 14)).ma)
        self.register("EMA", lambda c, df, p: vbt.MA.run(c, window=p.get_int("period", 14), ewm=True).ma)
        ...
        # Oscillators
        self.register("RSI", lambda c, df, p: vbt.RSI.run(c, window=p.get_int("period", 14)).rsi)
        ...
        self.register("ATR", lambda c, df, p: vbt.ATR.run(
        ...
        # Volume Indicators
        self.register("OBV", lambda c, df, p: vbt.OBV.run(df['close'], df['volume']).obv)
        ...
        self.register("TYPICAL_PRICE", lambda c, df, p: (df['high'] + df['low'] + df['close']) / 3)
        self.register("MEDIAN_PRICE", lambda c, df, p: (df['high'] + df['low']) / 2)
        
        # Constant
        self.register("CONSTANT", lambda c, df, p: pd.Series(p.get_float("value", 0), index=c.index))
    
    # Indicator implementations
    def _dema(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 14)
        ema1 = close.ewm(span=period, adjust=False).mean()
        ema2 = ema1.ewm(span=period, adjust=False).mean()
        return 2 * ema1 - ema2
    
    def _tema(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 14)
        ema1 = close.ewm(span=period, adjust=False).mean()
        ema2 = ema1.ewm(span=period, adjust=False).mean()
    ...
    def _wma(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 14)
        weights = np.arange(1, period + 1)
        return close.rolling(period).apply(lambda x: np.dot(x, weights) / weights.sum(), raw=True)
    
    def _macd(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
    ...
    def _macd_signal(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        fast = params.get_int("shortPeriod", 12)
        slow = params.get_int("longPeriod", 26)
    ...
    def _macd_hist(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        fast = params.get_int("shortPeriod", 12)
        slow = params.get_int("longPeriod", 26)
        signal = params.get_int("signalPeriod", 9)
        macd = vbt.MACD.run(close, fast_window=fast, slow_window=slow, signal_window=signal)
    ...
    def _stoch_k(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 14)
        stoch = vbt.STOCH.run(ohlcv['high'], ohlcv['low'], ohlcv['close'], k_window=period)
        return stoch.percent_k
    
    def _stoch_d(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        k_period = params.get_int("kPeriod", 14)
        d_period = params.get_int("dPeriod", 3)
        stoch = vbt.STOCH.run(ohlcv['high'], ohlcv['low'], ohlcv['close'], k_window=k_period, d_window=d_period)
        return stoch.percent_d
    
    def _cci(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
    ...
    def _cmo(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 14)
        delta = close.diff()
    ...
    def _williams_r(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 14)
        highest = ohlcv['high'].rolling(period).max()
        lowest = ohlcv['low'].rolling(period).min()
        return -100 * (highest - close) / (highest - lowest)
    
    def _roc(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 12)
        return 100 * (close - close.shift(period)) / close.shift(period)
    
    def _dpo(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 20)
        shift = period // 2 + 1
        sma = close.rolling(period).mean()
        return close.shift(shift) - sma
    
    def _adx(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 14)
    ...
    def _plus_di(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 14)
        high, low = ohlcv['high'], ohlcv['low']
        plus_dm = high.diff()
        minus_dm = -low.diff()
    ...
    def _minus_di(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 14)
        high, low = ohlcv['high'], ohlcv['low']
        minus_dm = -low.diff()
        plus_dm = high.diff()
    ...
    def _aroon_up(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
    ...
    def _aroon_down(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 25)
    ...
    def _parabolic_sar(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
    ...
    def _bbands_upper(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 20)
        mult = params.get_float("multiplier", 2.0)
        bb = vbt.BBANDS.run(close, window=period, alpha=mult)
        return bb.upper
    
    def _bbands_middle(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 20)
        bb = vbt.BBANDS.run(close, window=period)
        return bb.middle
    
    def _bbands_lower(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 20)
        mult = params.get_float("multiplier", 2.0)
        bb = vbt.BBANDS.run(close, window=period, alpha=mult)
        return bb.lower
    
    def _donchian_upper(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 20)
        return ohlcv['high'].rolling(period).max()
    
    def _donchian_lower(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 20)
        return ohlcv['low'].rolling(period).min()
    
    def _vwap(self, close: pd.Series, ohlcv: pd.DataFrame, params: IndicatorParams) -> pd.Series:
        period = params.get_int("period", 14)
        tp = (ohlcv['high'] + ohlcv['low'] + ohlcv['close']) / 3
        return (tp * ohlcv['volume']).rolling(period).sum() / ohlcv['volume'].rolling(period).sum()
