...
class TestRedisManager(unittest.TestCase):
    ...
    def test_connect_ping_fails_raises_connection_error(
        ...
        with self.assertRaises(redis.exceptions.ConnectionError) as context:
            ...
            )  # ConnectionError raised by _connect
    ...
    def test_get_client_not_initialized_attempts_reconnect_success(
        ...
        mock_first_attempt.ping.side_effect = redis.exceptions.ConnectionError(
        ...
        manager = RedisManager(
            host="localhost", port=6379
        )  # Initial connection fails silently in __init__ for this test
        ...
        self.assertEqual(
            mock_redis_client_constructor.call_count, 2
        )  # Init + get_client
    ...
    def test_get_client_not_initialized_reconnect_fails(
        ...
        manager = RedisManager(
            host="localhost", port=6379
        )  # Initial connection fails silently
        ...
        self.assertEqual(
            mock_redis_client_constructor.call_count, 6
        )  # __init__ (5 attempts) + get_client (1 attempt)
