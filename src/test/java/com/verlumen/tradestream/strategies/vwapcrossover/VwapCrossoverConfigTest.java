...
public class VwapCrossoverConfigTest {
  ...
  public void setUp() throws Exception {
    config = StrategyConfigLoader.loadResource("strategies/vwap_crossover.yaml");
    ...
    for (int i = 0; i < 100; i++) {
      ...
      series.addBar(
          new BaseBar(Duration.ofMinutes(1), now.plusMinutes(i).toInstant().minus(Duration.ofMinutes(1)), now.plusMinutes(i).toInstant(), DecimalNum.valueOf(price), DecimalNum.valueOf(price + 2), DecimalNum.valueOf(price - 2), DecimalNum.valueOf(price), DecimalNum.valueOf(1000.0 + i * 10), DecimalNum.valueOf(0), 0));
    }
  }
  ...
  public void createStrategy_returnsValidStrategy() throws Exception {
    ...
    assertThat(strategy.getName()).isEqualTo("VWAP_CROSSOVER");
  }
  ...
  public void createParameters_fromChromosomes_succeeds() throws Exception {
    ImmutableList<NumericChromosome<?, ?>> chromosomes =
        ImmutableList.of(IntegerChromosome.of(10, 30), IntegerChromosome.of(5, 20));
