...
abstract class TiingoCryptoFetcherFn(
    @Suppress("UNUSED_PARAMETER") @com.google.inject.assistedinject.Assisted
    protected val granularity: Duration,
    @Suppress("UNUSED_PARAMETER") @com.google.inject.assistedinject.Assisted
...
class StatefulTestTiingoCryptoFetcherFnImpl(
  ...
  fun process(context: ProcessContext) {
    val inputKv = context.element() as KV<String, Void?>
    val symbol = inputKv.key
    synchronized(emittedSymbols) { // Synchronize access if multiple threads could process elements
...
class TiingoCryptoCandleTransformTest {
  ...
  private val candleBtc1 = Candle.newBuilder()
      .setOpen(10000.0)
      .setHigh(10100.0)
      .setLow(9900.0)
      .setClose(10050.0)
      .setVolume(100.0)
      .setTimestamp(com.google.protobuf.util.Timestamps.fromMillis(now.millis))
      .setCurrencyPair(btcUsd.symbol())
      .build()
  private val candleEth1 = Candle.newBuilder()
      .setOpen(300.0)
      .setHigh(305.0)
      .setLow(295.0)
      .setClose(302.0)
  ...
  fun setUp() {
    ...
    try {
      coderRegistry.getCoder(CurrencyPair::class.java)
    } catch (e: Exception) {
      coderRegistry.registerCoderForClass(CurrencyPair::class.java, 
          SerializableCoder.of(CurrencyPair::class.java))
    }
    try {
      coderRegistry.getCoder(Candle::class.java)
    } catch (e: Exception) {
      coderRegistry.registerCoderForClass(Candle::class.java, 
          SerializableCoder.of(Candle::class.java))
    }
  }
