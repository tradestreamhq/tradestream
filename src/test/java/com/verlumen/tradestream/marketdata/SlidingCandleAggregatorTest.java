...
public class SlidingCandleAggregatorTest {
  ...
  public void testAggregateSingleTrade() {
    ...
    PAssert.that(
            pipeline
                .apply(Create.of(KV.of("BTC/USD", trade)))
                .apply(
                    new SlidingCandleAggregator(
                        Duration.standardMinutes(1), Duration.standardSeconds(30))))
        .satisfies(
            iterable -> {
  ...
  public void testAggregateMultipleTradesSameWindow() {
    ...
    PAssert.that(
            pipeline
                .apply(Create.of(KV.of("BTC/USD", trade1), KV.of("BTC/USD", trade2)))
                .apply(
                    new SlidingCandleAggregator(
                        Duration.standardMinutes(1), Duration.standardSeconds(30))))
        .satisfies(
            iterable -> {
              // Since the Combine.perKey may output multiple elements (if the same key appears in
              // multiple windows)
              ...
              for (KV<String, Candle> kv : iterable) {
                if ("BTC/USD".equals(kv.getKey())) {
                  Candle candle = kv.getValue();
                  // For the window that contains both trades, the open should be 10000 and close
                  // 10100.
  ...
  public void testAggregateNoTrades() {
    ...
    PAssert.that(
            pipeline
                .apply(Create.empty(KvCoder.of(StringUtf8Coder.of(), ProtoCoder.of(Trade.class))))
                .apply(
                    new SlidingCandleAggregator(
                        Duration.standardMinutes(1), Duration.standardSeconds(30))))
        .empty();
