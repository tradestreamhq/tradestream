...
public class PublishTradeSignalsTest {
  ...
  public void testPublishTradeSignals_whenSignalIsActionable() {
    ...
    TradeSignal buySignal =
        TradeSignal.newBuilder().setType(TradeSignal.TradeSignalType.BUY).setPrice(100.0).build();

    PCollection<KV<String, TradeSignal>> input =
        pipeline.apply(Create.of(KV.of("AAPL", buySignal)));
  ...
  public void testPublishTradeSignals_logsErrorOnException() {
    ...
    TradeSignal sellSignal =
        TradeSignal.newBuilder().setType(TradeSignal.TradeSignalType.SELL).setPrice(200.0).build();

    PCollection<KV<String, TradeSignal>> input =
        pipeline.apply(Create.of(KV.of("GOOGL", sellSignal)));
  ...
  /**
   * A fake implementation of TradeSignalPublisher for testing purposes. This fake records the
   * signals that were published and can optionally throw an exception.
   */
