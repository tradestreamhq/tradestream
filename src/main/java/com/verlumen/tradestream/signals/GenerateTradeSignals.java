...
public final class GenerateTradeSignals
    extends PTransform<
        PCollection<KV<String, StrategyState>>, PCollection<KV<String, TradeSignal>>> {
  ...
  @Override
  public PCollection<KV<String, TradeSignal>> expand(PCollection<KV<String, StrategyState>> input) {

    return input.apply("GenerateSignalsFromOptimizedStrategies", ParDo.of(generateSignalsDoFn));
  }

  /**
   * Stateful DoFn that maintains the last signal and generates new signals based on the optimized
   * strategy.
   */
  private static class GenerateSignalsDoFn
    ...
    public void processElement(
      ...
      if (lastSignal == null) {
        lastSignal = TradeSignal.newBuilder().setType(TradeSignal.TradeSignalType.NONE).build();
      }
      ...
      if (candles == null || candles.isEmpty()) {
        logger.atWarning().log(
            "No candle data available for key: %s. Cannot generate signal.", key);
        return;
      }
    ...
    private TradeSignal generateSignal(
      ...
      try {
        ...
        signalBuilder
            .setType(TradeSignal.TradeSignalType.ERROR)
