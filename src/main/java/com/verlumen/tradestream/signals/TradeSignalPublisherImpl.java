...
/**
 * Kafka-based implementation of TradeSignalPublisher that publishes trade signals to a specified
 * topic.
 */
final class TradeSignalPublisherImpl implements TradeSignalPublisher {
  ...
  TradeSignalPublisherImpl(
      Supplier<KafkaProducer<String, byte[]>> kafkaProducer, @Assisted String topic) {
  ...
  public void publish(TradeSignal signal) {
    logger.atInfo().log(
        "Publishing trade signal to topic %s. Type=%s, Timestamp=%s, Price=%f",
        topic,
        signal.getType(),
