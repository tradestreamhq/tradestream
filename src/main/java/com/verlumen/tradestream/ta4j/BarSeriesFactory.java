 ...
 *
 * <p>This interface defines methods for converting lists of {@link
 * com.verlumen.tradestream.marketdata.Candle} objects into a {@link BarSeries} object, which
 * represents a sequence of trading bars used for technical analysis in TA4J-based systems.
 * Implementations of this interface are responsible for defining the conversion logic, including
 * mapping candle attributes to bar properties.
 *
 * <p>Example usage:
 *
 * <pre>{@code
...
public interface BarSeriesFactory extends Serializable {
   ...
   *
   * <p>The implementation of this method should convert each candle in the provided list into a
   * corresponding trading bar, aggregating these bars into a {@link BarSeries} suitable for
   * technical analysis. Implementations may assume that the list is sorted in chronological order.
   *
   ...
   *
   * <p>Internally, this method creates an {@link ImmutableList} from the provided list and then
   * delegates to {@link #createBarSeries(ImmutableList)}.
   *
