...
/**
 * Classifies the current market regime by analyzing ADX (trend strength),
 * ATR (volatility relative to price), and Bollinger Band width (price compression).
 *
 * <p>Classification logic:
 * <ul>
 *   <li>ADX > adxTrendThreshold => trending (direction from +DI vs -DI)</li>
 *   <li>ADX <= adxTrendThreshold and high ATR% => high-volatility (choppy)</li>
 *   <li>ADX <= adxTrendThreshold and low BB width => low-volatility (quiet)</li>
 *   <li>Otherwise => ranging</li>
 * </ul>
 */
public final class MarketRegimeDetector implements Serializable {
    ...
    private static final ImmutableMap<MarketRegime, ImmutableList<String>> REGIME_STRATEGIES =
            ImmutableMap.<MarketRegime, ImmutableList<String>>builder()
                    .put(MarketRegime.TRENDING_UP, ImmutableList.of(
                            "ADX_DMI", "MACD_CROSSOVER", "EMA_MACD", "PARABOLIC_SAR",
                            "TRIPLE_EMA_CROSSOVER", "ICHIMOKU_CLOUD", "MOMENTUM_SMA_CROSSOVER"))
                    .put(MarketRegime.TRENDING_DOWN, ImmutableList.of(
                            "ADX_DMI", "MACD_CROSSOVER", "EMA_MACD", "PARABOLIC_SAR",
                            "ATR_TRAILING_STOP", "VOLATILITY_STOP"))
                    .put(MarketRegime.RANGING, ImmutableList.of(
                            "BBAND_W_R", "RSI_EMA_CROSSOVER", "STOCHASTIC_RSI",
                            "SMA_RSI", "VWAP_MEAN_REVERSION", "PIVOT", "FIBONACCI_RETRACEMENTS"))
                    .put(MarketRegime.HIGH_VOLATILITY, ImmutableList.of(
                            "ATR_TRAILING_STOP", "VOLATILITY_STOP", "ATR_CCI",
                            "DONCHIAN_BREAKOUT", "VOLUME_BREAKOUT", "KELTNER_CHANNEL"))
                    .put(MarketRegime.LOW_VOLATILITY, ImmutableList.of(
                            "BBAND_W_R", "DONCHIAN_BREAKOUT", "RANGE_BARS",
                            "SMA_EMA_CROSSOVER", "DOUBLE_EMA_CROSSOVER"))
                    .buildOrThrow();
    ...
    public RegimeClassification classify(BarSeries series, int index) {
        ...
        checkArgument(index >= 0 && index < series.getBarCount(),
                "index %s out of range [0, %s)", index, series.getBarCount());
        ...
        double bbWidthPercent = bbMiddleValue > 0
                ? (bbUpperValue - bbLowerValue) / bbMiddleValue : 0;
    ...
    public RegimeChangeEvent detectChange(BarSeries series, int previousIndex, int currentIndex) {
        checkArgument(previousIndex < currentIndex,
                "previousIndex %s must be less than currentIndex %s", previousIndex, currentIndex);
