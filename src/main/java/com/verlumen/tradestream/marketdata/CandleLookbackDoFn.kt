...
class CandleLookbackDoFn(
    ...
    init {
        ...
        logger.atInfo().log(
            "Initialized CandleLookbackDoFn with lookbackSizes=%s, maxQueueSize=%s",
            this.lookbackSizes,
            this.maxQueueSize,
        )
    }
    ...
    fun processElement(
        ...
        logger.atFine().log(
            "Processing candle for key=%s, timestamp=%s",
            key,
            newCandle.timestamp,
        )
        ...
        if (queue == null) {
            ...
            logger.atFine().log(
                "Retrieved existing queue for key=%s, current size=%d/%d",
                key,
                queue.size,
    ...
    private fun processLookbacks(
        ...
        logger.atInfo().log(
            "Processing lookbacks for key=%s, queue size=%d/%d",
            key,
            currentSize,
            queue.remainingCapacity() + queue.size,
        )
        ...
        for (lookbackSize in lookbackSizes) {
            ...
            try {
                ...
                logger.atFine().log(
                    "Emitted lookback: key=%s, size=%d, from=%d to=%d",
                    key,
                    lookbackSize,
                    startIndex,
                    currentSize,
                ...
                logger.atSevere().withCause(e).log(
                    "Failed to process lookback: key=%s, size=%d",
                    key,
                    lookbackSize,
                )
            }
        ...
        if (skippedSizes.isNotEmpty()) {
            logger.atInfo().log(
                "Skipped lookback sizes for key=%s: %s (insufficient data, queue size=%d)",
                key,
                skippedSizes,
                currentSize,
            )
        ...
        logger.atInfo().log(
            "Lookback processing complete for key=%s: emitted=%d",
            key,
            emittedCount,
        )
    }
