 ...
 * pair. This version includes state management to fetch incrementally.
 */
  ...
  fun processElement(
    ...
    val startDate =
        if (lastStoredTimestamp != null) {
          ...
          logger.atInfo().log(
              "No previous state for %s. Using default start date: %s",
              currencyPair,
              DEFAULT_START_DATE)
          DEFAULT_START_DATE
        }
    ...
    val url =
        "$TIINGO_API_URL?tickers=$ticker&startDate=$startDate&resampleFreq=$resampleFreq&token=$apiKey"
    ...
    try {
      ...
      if (candles.isNotEmpty()) {
        ...
        for (candle in candles) {
          ...
          val candleMillis =
              candle.timestamp.getSeconds() * 1000 + candle.timestamp.getNanos() / 1_000_000
    ...
    if (latestEpochMillis > 0) {
      ...
      logger.atInfo().log(
          "Updated last timestamp state for %s to: %s",
          currencyPair,
          Instant.ofEpochMilli(latestEpochMillis))
      ...
      try {
        val startInstant =
            if (isDailyGranularity(granularity)) {
              ...
              try {
                ...
                LocalDate.parse(DEFAULT_START_DATE, TIINGO_DATE_FORMATTER_DAILY)
                    .atStartOfDay()
                    .toInstant(ZoneOffset.UTC)
              }
            }
        ...
        logger.atInfo().log(
            "Initial fetch for %s yielded no data. Setting state to start date: %s",
            currencyPair,
            startInstant)
      } catch (e: Exception) {
        logger
