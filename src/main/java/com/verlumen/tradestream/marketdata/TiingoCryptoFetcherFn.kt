...
class TiingoCryptoFetcherFn @Inject constructor(
    ...
    fun processElement(
        ...
        if (apiKey.isBlank()) {
            logger.atWarning()
                .log("Skipping Tiingo fetch for %s: Empty API key", currencyPair)
            return
        }
        ...
        if (latestEpochMillis > 0) {
            ...
            try {
                ...
                logger.atWarning().withCause(e)
                    .log("Could not parse start date '%s' to set initial state for %s",
    ...
    /**
     * Fill gaps between fetched candles using the last known candle as reference.
     * If lastKnownCandle is null, simply return fetched.
     */
    private fun fillMissingCandles(
        ...
        for (curr in fetched) {
            val currTime = Instant.ofEpochSecond(
                curr.timestamp.seconds,
                curr.timestamp.nanos.toLong()
            )
            var nextExpected = Instant.ofEpochSecond(
            ...
            while (nextExpected.isBefore(currTime)) {
                logger.atFine().log(
                    ...
                    curr.currencyPair, nextExpected, currTime
                )
