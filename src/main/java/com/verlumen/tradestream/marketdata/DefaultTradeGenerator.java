...
/**
 * DefaultTradeGenerator generates synthetic Trade messages for a given key. The generated trade has
 * zero volume and uses a default price. This transform ensures that even when there are no real
 * trades, a dummy trade is injected to trigger candle creation downstream.
 */
public class DefaultTradeGenerator
    extends PTransform<PCollection<KV<String, Void>>, PCollection<KV<String, Trade>>> {
  ...
  public PCollection<KV<String, Trade>> expand(PCollection<KV<String, Void>> input) {
    return input.apply(
        "GenerateDefaultTrades", ParDo.of(new DefaultTradeGeneratorFn(defaultPrice)));
  }
  ...
  public static class DefaultTradeGeneratorFn extends DoFn<KV<String, Void>, KV<String, Trade>> {
    ...
    public void processElement(
        @Element KV<String, Void> element, OutputReceiver<KV<String, Trade>> out) {
