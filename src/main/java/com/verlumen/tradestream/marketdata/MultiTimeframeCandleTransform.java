...
/**
 * MultiTimeframeCandleTransform branches a consolidated base candle stream into multiple timeframe
 * views. For each timeframe defined in the TimeFrame enum, it applies a buffering transform
 * (LastCandlesFn.BufferLastCandles) with a buffer size equal to the number of minutes specified.
 * Before flattening, each branch is explicitly re-windowed into GlobalWindows so that all branches
 * have the same windowing.
 *
 * <p>This transform assumes that the input is a consolidated candle stream (for example, from
 * CandleStreamWithDefaults) where each key appears only once.
 */
public class MultiTimeframeCandleTransform
    extends PTransform<
        PCollection<KV<String, Candle>>, PCollection<KV<String, ImmutableList<Candle>>>> {

  private static final FluentLogger logger = FluentLogger.forEnclosingClass();

  @Override
  public PCollection<KV<String, ImmutableList<Candle>>> expand(
      PCollection<KV<String, Candle>> input) {
    ...
    PCollection<KV<String, Iterable<Candle>>> grouped =
    ...
    PCollection<KV<String, Candle>> consolidated =
        grouped.apply(
            "ConsolidateCandles",
            ParDo.of(
                new DoFn<KV<String, Iterable<Candle>>, KV<String, Candle>>() {

                  @ProcessElement
                  public void processElement(ProcessContext c) {
                    KV<String, Iterable<Candle>> element = c.element();
                    logger.atFine().log(
                        "Consolidating candles for key: %s. Iterable size unknown (stream).",
                        element.getKey());
                    ...
                    if (last != null) {
                      logger.atFine().log(
                          "Consolidation complete for key: %s. Last candle: %s",
                          element.getKey(), last);
                      c.output(KV.of(element.getKey(), last));
                    } else {
                      logger.atFine().log(
                          "No valid candle found to consolidate for key: %s", element.getKey());
                    }
                  }
                }));
    ...
    logger.atInfo().log(
        "Creating branches for each TimeFrame. Available timeframes: %s", TimeFrame.values());
    ...
    for (TimeFrame tf : TimeFrame.values()) {
      ...
      logger.atInfo().log(
          "Creating branch '%s' with bufferSize=%d (minutes).", branchName, bufferSize);

      PCollection<KV<String, ImmutableList<Candle>>> branch =
          consolidated
              .apply(branchName, ParDo.of(new LastCandlesFn.BufferLastCandles(bufferSize)))
              // 4. Re-window each branch into GlobalWindows.
              .apply(
                  "Rewindow_" + branchName,
