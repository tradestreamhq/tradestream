...
/**
 * CandleCombineFn aggregates Trade messages into a Candle. This CombineFn can be used with
 * Combine.perKey() in Beam pipelines.
 */
public class CandleCombineFn
    extends Combine.CombineFn<Trade, CandleCombineFn.CandleAccumulator, Candle> {
  private static final FluentLogger logger = FluentLogger.forEnclosingClass();
  private static final double ZERO = 0.0;

  @Override
  public Coder<CandleAccumulator> getAccumulatorCoder(
      CoderRegistry registry, Coder<Trade> inputCoder) {
  ...
  public CandleAccumulator addInput(CandleAccumulator accumulator, Trade trade) {
    ...
    if (accumulator.firstTrade) {
      ...
      logger.atFiner().log(
          "Initialized accumulator with first real trade. Accumulator now: %s", accumulator);
    } else {
  ...
  public static class CandleAccumulator {
    ...
    public String toString() {
      return String.format(
          "CandleAccumulator{open=%.4f, high=%.4f, low=%.4f, close=%.4f, volume=%.4f,"
              + " firstTrade=%b, openTs=%s, closeTs=%s, pair=%s}",
          open, high, low, close, volume, firstTrade, openTimestamp, closeTimestamp, currencyPair);
    }
  }

  /**
   * Custom coder for CandleAccumulator to enable serialization/deserialization in the Beam
   * pipeline.
   */
