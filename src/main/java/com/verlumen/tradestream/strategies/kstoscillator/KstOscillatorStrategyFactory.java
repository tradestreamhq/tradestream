 ...
 * and smoothing to create a comprehensive momentum indicator. The strategy generates buy signals
 * when the KST crosses above its signal line and sell signals when it crosses below.
 */
public final class KstOscillatorStrategyFactory implements StrategyFactory<KstOscillatorParameters> {
  ...
  public Strategy createStrategy(BarSeries barSeries, KstOscillatorParameters parameters) {
    ...
    KstOscillatorIndicator kstIndicator = new KstOscillatorIndicator(
        barSeries,
   ...
   * 
   * The KST Oscillator is calculated as:
   * 1. Calculate ROC for four different periods (typically 10, 15, 20, 30)
   * 2. Apply SMA smoothing to each ROC (typically 10, 10, 10, 15)
   * 3. Weight the smoothed ROCs: (ROC1 * 1) + (ROC2 * 2) + (ROC3 * 3) + (ROC4 * 4)
   * 4. KST = Weighted sum of smoothed ROCs
   */
  private static class KstOscillatorIndicator extends CachedIndicator<org.ta4j.core.num.Num> {
    ...
    public KstOscillatorIndicator(BarSeries series, int rma1Period, int rma2Period, int rma3Period, int rma4Period) {
      super(series);
    ...
    protected org.ta4j.core.num.Num calculate(int index) {
      ...
      org.ta4j.core.num.Num weightedSum = 
          smoothedRoc1.getValue(index).multipliedBy(numOf(1))
    ...
    public int getUnstableBars() {
      ...
      return Math.max(Math.max(Math.max(roc1.getUnstableBars(), roc2.getUnstableBars()),
                               Math.max(roc3.getUnstableBars(), roc4.getUnstableBars())),
                      Math.max(Math.max(smoothedRoc1.getUnstableBars(), smoothedRoc2.getUnstableBars()),
