...
public final class PivotParamConfig implements ParamConfig {
  private static final Logger logger = Logger.getLogger(PivotParamConfig.class.getName());

  private static final ImmutableList<ChromosomeSpec<?>> SPECS =
      ImmutableList.of(
          ChromosomeSpec.ofInteger(5, 50) // period
          );
  ...
  public Any createParameters(ImmutableList<? extends NumericChromosome<?, ?>> chromosomes) {
    ...
    PivotParameters parameters = PivotParameters.newBuilder().setPeriod(period).build();
  ...
  private Any getDefaultParameters() {
    return Any.pack(PivotParameters.newBuilder().setPeriod(20).build());
  }
}
