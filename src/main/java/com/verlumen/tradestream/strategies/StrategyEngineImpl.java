...
import com.verlumen.tradestream.strategies.CandleBuffer;
import com.verlumen.tradestream.strategies.TradeSignal;
...
final class StrategyEngineImpl implements StrategyEngine {
    ...
    private volatile TradeSignal lastSignal = TradeSignal.newBuilder()
    ...
    private void initializeStrategyRecords() {
        ...
        for (StrategyType type : StrategyType.values()) {
            strategyRecords.put(
                type, 
                new StrategyRecord(type, null, Double.NEGATIVE_INFINITY)
            );
        }
        ...
        for (StrategyType strategyType : strategyRecords.keySet()) {
            ...
            try {
                ...
                updateStrategyRecord(
                    strategyType,
                    response.getBestStrategyParameters(),
                    response.getBestScore()
                );
            } catch (Exception e) {
    ...
    private void updateStrategyRecord(
            ...
            com.google.protobuf.Any parameters,
            double score) {
        strategyRecords.put(
            strategyType,
            new StrategyRecord(strategyType, parameters, score)
        );
    }
    ...
    private void selectBestStrategy() {
        ...
        if (bestRecord.strategyType() != currentStrategyType) {
            try {
                Strategy newStrategy = strategyManager.createStrategy(
                    candleBuffer.toBarSeries(),
                    bestRecord.strategyType(),
