 ...
 * Implementations of this interface are responsible for instantiating a specific type of trading
 * strategy based on configuration data, often represented as a Protocol Buffer message.
 *
...
public interface StrategyFactory<T extends Message> extends Serializable {
  ...
  /**
   * Creates a Ta4j Strategy object from the provided {@link BarSeries} and parameters. This is a
   * convenience method that unpacks the parameters from an {@link Any} message before invoking
   * {@link #createStrategy(BarSeries, Message)}.
   *
   * @param series The {@link BarSeries} to associate with the created strategy.
   * @param parameters The parameters for the strategy wrapped in an {@link Any} message.
   * @return The created {@link Strategy} object.
   * @throws InvalidProtocolBufferException If there is an error when unpacking the {@link Any}
   *     message.
   */
  default Strategy createStrategy(BarSeries series, Any parameters)
