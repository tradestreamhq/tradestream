...
final class StrategyStateFactoryImpl implements StrategyState.Factory {
  ...
  /**
   * Implementation of the {@link StrategyState} interface. This class is serializable to support
   * Beam state APIs.
   */
  private static record StrategyStateImpl(
    ...
    private static StrategyStateImpl create(StrategyManager strategyManager) {
      ...
      for (StrategyType type : strategyManager.getStrategyTypes()) {
        strategyRecords.put(
            type, StrategyRecord.create(type, strategyManager.getDefaultParameters(type)));
      }
      return new StrategyStateImpl(
          strategyManager, strategyRecords, new AtomicReference<>(DEFAULT_STRATEGY_TYPE));
    }
