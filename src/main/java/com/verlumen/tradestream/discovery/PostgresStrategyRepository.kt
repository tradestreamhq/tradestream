...
class PostgresStrategyRepository @Inject constructor(
    ...
    override fun findBySymbol(symbol: String): List<DiscoveredStrategy> {
        ...
        dataSource.connection.use { conn ->
            val sql = "SELECT symbol, strategy_type, parameters, current_score, strategy_hash, discovery_symbol, discovery_start_time, discovery_end_time FROM Strategies WHERE symbol = ?"
    ...
    override fun findAll(): List<DiscoveredStrategy> {
        val result = mutableListOf<DiscoveredStrategy>()
        dataSource.connection.use { conn ->
            val sql = "SELECT symbol, strategy_type, parameters, current_score, strategy_hash, discovery_symbol, discovery_start_time, discovery_end_time FROM Strategies"
            conn.prepareStatement(sql).use { stmt ->
                val rs = stmt.executeQuery()
                while (rs.next()) {
                    result.add(mapRowToDiscoveredStrategy(rs))
    ...
    private fun mapRowToDiscoveredStrategy(rs: ResultSet): DiscoveredStrategy {
        ...
        val strategy = com.verlumen.tradestream.strategies.Strategy.newBuilder()
            .setTypeValue(com.verlumen.tradestream.strategies.StrategyType.valueOf(strategyType).number)
            .setParameters(parametersAny)
            .build()

        // Build DiscoveredStrategy proto
        return DiscoveredStrategy.newBuilder()
            .setStrategy(strategy)
            .setScore(score)
            .setSymbol(symbol)
            .setStartTime(com.google.protobuf.Timestamp.newBuilder().setSeconds(startTime.time / 1000).build())
            .setEndTime(com.google.protobuf.Timestamp.newBuilder().setSeconds(endTime.time / 1000).build())
            .build()
    }
} 