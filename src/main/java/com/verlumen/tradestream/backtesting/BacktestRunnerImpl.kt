 ...
 */
internal class BacktestRunnerImpl @Inject constructor(
    ...
    override fun runBacktest(request: BacktestRequest): BacktestResult {
        ...
        val strategy = strategyManager.createStrategy(
            series, request.strategy.type, request.strategy.parameters
        )
    ...
    private fun calculateAnnualizedReturn(series: BarSeries, record: TradingRecord): Double {
    ...
    private fun calculateSharpeRatio(returns: Double, volatility: Double): Double {
        val riskFreeRate = 0.02 // Assume 2% risk-free rate
        return if (volatility == 0.0) 0.0 else (returns - riskFreeRate) / volatility
    }

    private fun calculateSortinoRatio(series: BarSeries, record: TradingRecord): Double {
        // Simplified Sortino calculation using only negative returns
        val negativeReturns = (1 until series.barCount)
            .map { i ->
