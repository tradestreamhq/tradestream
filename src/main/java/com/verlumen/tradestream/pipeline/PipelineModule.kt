...
@AutoValue
abstract class PipelineModule : AbstractModule() {
  ...
  companion object {
    fun create(config: PipelineConfig): PipelineModule {
      return AutoValue_PipelineModule(config)
    }
  }

  abstract fun config(): PipelineConfig
  ...
  @Provides
  fun providePipelineConfig(): PipelineConfig {
    return config()
  }
}
